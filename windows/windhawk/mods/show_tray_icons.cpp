// ==WindhawkMod==
// @id              taskbar-notification-icons-show-all-fork
// @name            Always show all taskbar tray icons - Fork
// @description     Restore the missing Windows option to always show all notification (tray) icons (Windows 11 only)
// @version         1.1
// @author          rehanhaider
// @github          https://github.com/rehanhaider
// @twitter         https://twitter.com/rehanhaider
// @homepage        https://rehanhaider.com/
...
bool IsIconExcluded(const std::wstring& entry) {
    return g_settings.exclusions.find(entry) != g_settings.exclusions.end();
}
...
LONG WINAPI RegSetValueExW_Hook(HKEY hKey,
    ...
    if (g_settings.mode == Mode::showAll && lpValueName &&
        ...
        if (!entry.empty()) {
            if (IsIconExcluded(entry)) {
                Wh_Log(L"Allowing write to IsPromoted for excluded icon: %s", entry.c_str());
...
LONG WINAPI RegGetValueW_Hook(HKEY hkey,
    ...
    if (!lpSubKey && lpValue && (dwFlags & RRF_RT_REG_DWORD) && pvData &&
        ...
        if (!entry.empty()) {
            ...
            if (IsIconExcluded(entry)) {
                Wh_Log(L"Icon %s is excluded, using original behavior", entry.c_str());
...
void ParseExclusions(PCWSTR exclusionsStr) {
    g_settings.exclusions.clear();
    
    if (!exclusionsStr || wcslen(exclusionsStr) == 0) {
        return;
    }
    
    std::wstring exclusionsString(exclusionsStr);
    auto exclusionsList = SplitStringView(exclusionsString, L',');
    
    for (auto& exclusion : exclusionsList) {
        // Trim whitespace
        size_t start = exclusion.find_first_not_of(L" \t");
        if (start == std::wstring::npos) continue;
        
        size_t end = exclusion.find_last_not_of(L" \t");
        std::wstring trimmed = exclusion.substr(start, end - start + 1);
        
        if (!trimmed.empty()) {
            g_settings.exclusions.insert(trimmed);
            Wh_Log(L"Added exclusion: %s", trimmed.c_str());
        }
    }
    
    Wh_Log(L"Loaded %zu exclusions", g_settings.exclusions.size());
}

void LoadSettings() {
    PCWSTR mode = Wh_GetStringSetting(L"mode");
    g_settings.mode = Mode::showAll;
    if (mode && _wcsicmp(mode, L"showNew") == 0) {
        g_settings.mode = Mode::showNew;
    }
    Wh_FreeStringSetting(mode);
    
    PCWSTR exclusions = Wh_GetStringSetting(L"exclusions");
    ParseExclusions(exclusions);
    Wh_FreeStringSetting(exclusions);
}
